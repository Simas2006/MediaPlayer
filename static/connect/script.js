...
function doLogin() {
  ...
  if ( ! url.startsWith("http") && location.search != "?server" ) url = "http://" + url;
...
window.onload = function() {
  ...
  fs.readFile(__dirname + "/../lang/" + localStorage.getItem("language"),function(err,data) {
    ...
    document.getElementById("title").innerText = location.search == "?server" ? lang.stream_server_title : lang.stream_client_title;
    document.getElementById("ip_text").innerText = location.search == "?server" ? lang.port : lang.ip_address;
